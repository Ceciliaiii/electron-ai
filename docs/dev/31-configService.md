...
## ConfigService
配置服务，负责管理应用的配置项，包括主题模式、高亮色、语言、字体大小、最小化到托盘、provider配置、默认大模型等：
 - 在配置变更的逻辑上，采用了观察者模式，当配置项发生变化时，`ConfigService` 通知所有监听者 `_listeners`，并执行监听者的回调函数 `listener(config)`。  
 ```
调用 set/update 修改配置 → 
自动执行 _saveConfig 保存 → 
_saveConfig 调用 _notifyListeners → 
_notifyListeners 遍历执行 onConfigChange 注册的回调 listener(config)

autoSave可以控制是否自动保存config到本地
 ```
 - 返回 config 时进行深拷贝，避免污染源数据；以后需要返回引用类型数据时，都尽量如此操作。  
 - 通过创建本地目录并且将配置写入 config.json，每次加载配置时都读取该文件，实现持久化配置。详情见 `_loadConfig` 和 `_saveConfig`。  
```ts
// main/service/ConfigService.ts
