# Message 存储
将用户发送、更新、删除的 message 关联本地内存 store 与数据库 dataBase 中。

## 去重方法
由于可能重复查询 message，所以需要对 message 进行去重处理。
```ts
// common/utils.ts
...
## messageStore
编写初始化、增删改查的逻辑：  
 - 初始化：去重与避开已存在store的消息加载；
 - 增删改：对 store 和 dataBase 的同步操作
```ts
...
将此应用到 main 对话区域：  
 - 跳转同一对话项时，直接 next 放行；
 - 跳转不同对话项时，先加载对应对话项的 message，再 next 放行；
```ts
